...
import { fetchTripByTripIdAsync } from './redux/reducers/trips/thunks'
...
export function ShareInterface() {
  ...
  useEffect(() => {
    async function fetchTrip() {
      try {
        const trips = await dispatch(fetchTripByTripIdAsync(id)).unwrap()
        if (trips.length !== 1) {
          throw Error('many trips received, this should not have happened')
        } else {
          const trip = trips[0]
          dispatch(setActiveTripId(trip._id))
          dispatch(fetchStagesByTripIdAsync(trip._id))
          dispatch(
